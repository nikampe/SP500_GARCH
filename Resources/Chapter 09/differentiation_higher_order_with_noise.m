...
% Caveat: higher order schemes will always get perfect solutions to
% polynomials!
 f =@(x)x.^4+0.000000001*randn;
 g1=@(x)4*x.^3;   g2=@(x)12*x.^2;
 g3=@(x)24*x;     x0=1;

 f =@(x)sin(x)+0.000000001*randn;
 g1=@(x)cos(x);   g2=@(x)-sin(x);
 g3=@(x)-cos(x);     x0=1;
